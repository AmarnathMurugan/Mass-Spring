...
//
{
    ...
    std::cout << "\n";
    std::cout << "TET_MESH_BOUNDARY\n";
    std::cout << "  C++ version\n";
    std::cout << "  Read files defining a tet mesh dataset.\n";
    std::cout << "\n";
    std::cout << "  Determine the faces that form the boundary;\n";
    std::cout << "  write a boundary node mask file;\n";
    std::cout << "  write a boundary node file;\n";
    std::cout << "  write a boundary element file,\n";
    std::cout << "  defining the boundary as a TRI_SURFACE.\n";
    std::cout << "\n";
    ...
    std::cout << "\n";
    std::cout << "  Read the header of \"" << node_filename << "\".\n";

    std::cout << "\n";
    std::cout << "  Spatial dimension = " << dim_num << "\n";
    std::cout << "  Number of nodes   = " << node_num << "\n";

    // first line containing metadata is also considered to be part of mesh, decrement to account for this 
    dim_num = 3;
    node_xyz = r8mat_data_read(node_filename, dim_num, node_num);

    std::cout << "\n";
    std::cout << "  Read the data in \"" << node_filename << "\".\n";

    r8mat_transpose_print_some(dim_num, node_num,
        node_xyz, 1, 7754, dim_num, 7759, "  First 5 nodes:");
    //
    //  Read the elements.
    //
    ...
    std::cout << "\n";
    std::cout << "  Read the header of \"" << element_filename << "\".\n";

    if (element_order != 4 && element_order != 10)
    {
        std::cout << "\n";
        std::cout << "TET_MESH_BOUNDARY - Fatal error!\n";
        std::cout << "  The elements must have order 4 or 10.\n";
        return 1;
    }

    std::cout << "\n";
    std::cout << "  Element order = " << element_order << "\n";
    std::cout << "  Number of tetras  = " << element_num << "\n";

    element_node = i4mat_data_read(element_filename, element_order,
        element_num);

    std::cout << "\n";
    std::cout << "  Read the data in \"" << element_filename << "\".\n";

    i4mat_transpose_print_some(element_order, element_num,
        element_node, 1, 32954, element_order, 32959, "  First 5 elements:");
    //
    ...
    std::cout << "\n";
    std::cout << "  Number of faces is " << 4 * element_num << "\n";
    std::cout << "  Number of boundary faces is " << boundary_element_num << "\n";
    std::cout << "  Number of boundary nodes is " << boundary_node_num << "\n";

    double* rearranged_node_xyz = new double[3 * node_num];

    int boundary_node_begin = 0;
    int interior_node_begin = boundary_node_num;

    std::unordered_map<int,int> old_to_new_node_map;
    
    for (int i = 0; i < node_num; i++)
    {
		if (boundary_node_mask[i] == 1)
		{
			for (int j = 0; j < 3; j++)
			{
				rearranged_node_xyz[3 * boundary_node_begin + j] = node_xyz[3 * i + j];
			}
            old_to_new_node_map[i] = boundary_node_begin;
			boundary_node_begin++;
		}
		else
		{
			for (int j = 0; j < 3; j++)
			{
				rearranged_node_xyz[3 * interior_node_begin + j] = node_xyz[3 * i + j];
			}
            old_to_new_node_map[i] = interior_node_begin;
			interior_node_begin++;
		}
    }

    for (int i = 0; i < element_order * element_num; i++)
    {
        element_node[i] = old_to_new_node_map[element_node[i]];
    }

    delete [] node_xyz;
    old_to_new_node_map.clear();

    node_xyz = rearranged_node_xyz;

    for (int i = 0; i < node_num; i++)
    {
        if(i<boundary_node_num)
		{
			boundary_node_mask[i] = 1;
		}
		else
		{
    ...
    positions = Eigen::Map<Eigen::VectorXd>(node_xyz, 3 * node_num).cast<float>();
...
//  Parameters:
//
//    Input, std::string INPUT_FILENAME, the name of the input file.
//
...
//
{
    ...
    {
        std::cerr << "\n";
        std::cerr << "I4MAT_DATA_READ - Fatal error!\n";
        std::cerr << "  Could not open the input file: \"" << input_filename << "\"\n";
        return NULL;
    }
...
//
{
    *m = file_column_count(input_filename);
    ...
    {
        std::cerr << "\n";
        std::cerr << "I4MAT_HEADER_READ - Fatal error!\n";
...
void i4mat_transpose_print_some(int m, int n, int a[], int ilo, int jlo,
    ...
    //
    //    Input, std::string TITLE, a title for the matrix.
{
# define INCX 10
...
//
{
    ...
    //
    //    output << "  " << std::setw(24) << setprecision(16) << table[i+j*m];
    //
    for (j = 0; j < n; j++)
    {
        for (i = 0; i < m; i++)
...
//  Purpose:
//
//    S_TO_I4VEC reads an I4VEC from a std::string.
//
//  Licensing:
//
//    This code is distributed under the MIT license.
...
//  Parameters:
//
//    Input, std::string S, the std::string to be read.
//
//    Input, int N, the number of values expected.
//
//    Output, int IVEC[N], the values read from the std::string.
//
...
//  Purpose:
//
//    S_TO_R8VEC reads an R8VEC from a std::string.
//
//  Licensing:
//
//    This code is distributed under the MIT license.
...
//    Input, int N, the number of values expected.
//
//    Output, double RVEC[N], the values read from the std::string.
//
...
//  Purpose:
//
//    S_WORD_COUNT counts the number of "words" in a std::string.
//
//  Licensing:
//
//    This code is distributed under the MIT license.
...
//
//    Output, int S_WORD_COUNT, the number of "words" in the std::string.
